...
`/v1/suggestions` (typewriter placeholder words/titles) and `/v1/analyze` (plain-language
sentence explanation). It stores nothing but the bare word, a frequency count, and optional
...
| [`src/suggestion/suggestions.ts`](src/suggestion/suggestions.ts) | Builds the `/v1/suggestions` response: prompts, tolerant JSON-array extraction (`extractJsonArray`), and `parseSuggestionList` validation. |
